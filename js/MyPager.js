...
function writePager(result,page,webpage="index.php",paras=""){
	...
	var startPage=page-((page%5==0)?5:(page%5))+1;//有页码的起始页，比如第5页，5-5+1=1；第12页，13-3+1=11；
	var endPage=(startPage+4<=pages)?(startPage+4):pages;//结束页为起始页+4之后和总页数的比较中的较小值
	pagerHtml+="<li><a href='"+webpage+"?page="+(page-5<1?1:(page-5))+paras+"'><</a></li>";//上一页html，简单-5就行，程序会判断是否超过范围
	if(startPage>=1){
		for(var i=startPage;i<=endPage;i++){
			pagerHtml+="<li><a href='"+webpage+"?page="+i+paras+"'>"+i+"</a></li>";//中间页html
		}
	}	
	pagerHtml+="<li><a href='"+webpage+"?page="+(page+5>pages?pages:(page+5))+paras+"'>></a></li>";//下一页html，简单+5就行，程序会判断是否超过范围
